...
impl Opcode {
    ...
    pub(crate) fn parse_opcode(lex: &mut Lexer<SfzToken>) -> Option<Opcode> {
        ...
        match (opcode.as_str(), values) {
            ...
            ("key", _) => utils::check_u8_key(value).map(Opcode::key),
            ...
            ("pan", _) => utils::check_f32_between(value, -100., 100.).map(Opcode::pan),
            ("pitch_keycenter", _) => {
                utils::check_u8_key(value).map(Opcode::pitch_keycenter)
            }
            ("pitch_keytrack", _) => {
            ...
            ("sw_hikey", _) => utils::check_u8_key(value).map(Opcode::sw_hikey),
            ("sw_last", _) => utils::check_u8_between(value, 0, 127).map(Opcode::sw_last),
            ("sw_lokey", _) => utils::check_u8_key(value).map(Opcode::sw_lokey),
